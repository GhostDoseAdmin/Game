    ...
    public void Update()
    {
        ...
        if (NetworkDriver.instance.HOST)
        {
            ...
            if (timer >= interval)
            {
                ...
                if (HP > 0) { if (!NetworkDriver.instance.OFFLINE) { NetworkDriver.instance.sioCom.Instance.Emit("event", JsonConvert.SerializeObject($"{{'obj':'{gameObject.name}','type':'{HP}','event':'zozohp'}}"), false); } }
